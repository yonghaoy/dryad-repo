        ...
        try {
            if (dso instanceof Item) {
                ...
                // reassign the URL of the DOI
                    ...
                    DOI removedDOI = new DOI(doi.toString(), new URL(dspaceURL + "/tombstone"));
                    mint(removedDOI, true);
                }
    ...
    public DSpaceObject resolve(Context context, String identifier, String... attributes) {
        ...
        try {
            ...
            if (identifier != null && identifier.startsWith("doi:")) {
                ...
                log.warn("DOIIdentofierService: resolve() identfier: " + identifier);

                String urlString = ConfigurationManager.getProperty("doi.service.url") + "?lookup=" + identifier;

                log.warn("DOIIdentofierService: resolve() urlString: " + urlString);
                ...
                log.warn("DOIIdentofierService: resolve handle (before replace) ==>>" + handle);

                if (handle != null) {
                    // replace the first part of the handle with the one used by this Dspace instance
                    // reading it from dspace.cfg
                    // e.g.,     http://datadryad.org/handle/10255/dryad.125 (production)
                    //           http://dev.datadryad.org/handle/10255/dryad.125 (test)
                    // replace   http://datadryad.org/  with   http://dev.datadryad.org/
                    String dspaceURL = ConfigurationManager.getProperty("dspace.url");
                    handle = handle.substring(handle.indexOf("handle"));
                    handle = dspaceURL + '/' + handle;

                    log.warn("DOIIdentofierService: resolve handle (after replace) ==>>" + handle);

                    return HandleManager.resolveUrlToDSpaceObject(context, handle);
                }
            }
        } catch (Exception e) {
            log.warn(LogManager.getHeader(context, "Error while attempting to resolve doi", "Identifier: " + identifier));
        }

...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
	...
	<xsl:variable name="myscheme" 
		select="$meta[@element='request'][@qualifier='scheme']"/>
	<xsl:variable name="myHost" 
		select="$meta[@element='request'][@qualifier='serverName']"/>
	<xsl:variable name="myPort" 
		select="$meta[@element='request'][@qualifier='serverPort']"/>
		...
		<table class="ds-includeSet-table">
			...
			<!--xsl:if test=".//dim:field[@element='relation' and @qualifier='haspart']">
				<tr class="ds-table-row">
					<td>
						<span class="bold">
							<i18n:text>xmlui.DryadItemSummary.individualDataFiles</i18n:text>
						</span>
					</td>
					<td width="60%" class="data-file">   
						<xsl:for-each
							select=".//dim:field[@element='relation' and @qualifier='haspart']">
							<xsl:variable name="fileURL" select="string(.)"/>
							<nobr>
								<a>
									<xsl:attribute name="href">
										<xsl:call-template name="checkURL">
									...
									<xsl:if
										test="not($meta[@element='dryad'][@qualifier='fileTitle'])">
										<xsl:value-of select="$fileURL"/>
									</xsl:if>
									<xsl:for-each
										select="$meta[@element='dryad'][@qualifier='fileTitle']">
										<xsl:choose>
											<xsl:when test="contains(., concat($fileURL, '|'))">
												<xsl:value-of select="strings:split(., '|')[last()]"
												/>
											</xsl:when>
											<xsl:when
												test="contains(., $fileURL) and not(contains(., '|'))">
												<xsl:value-of select="$fileURL"/>
											</xsl:when>
										</xsl:choose>
									</xsl:for-each>
								...
								<nobr>
									<xsl:if test="$meta[@element='dryad'][@qualifier='file-view-1']">
										<xsl:variable name="fv"
											select="concat('file-view-', $index)"/>
										<xsl:variable name="fv-value"
											select="$meta[@element='dryad'][@qualifier=$fv]"/>
										<span
											title="The number of times someone has viewed the data file page"
											style="font-size: smaller; font-weight: bold;">
											<xsl:text>&#160;&#160;&#160;</xsl:text>
											<xsl:value-of select="$fv-value"/>
											<xsl:choose>
												<xsl:when test="string($fv-value) = '1'">
												<i18n:text>xmlui.DryadItemSummary.view</i18n:text>
												</xsl:when>
												<xsl:otherwise>
												<i18n:text>xmlui.DryadItemSummary.views</i18n:text>
											...
											<xsl:variable name="downloads"
												select="$meta[@element='dryad'][@qualifier='file-dl-1']"/>
											<xsl:if test="$downloads">
												<xsl:variable name="fdl"
												select="concat('file-dl-', $index)"/>
												<xsl:variable name="fdl-value"
												select="$meta[@element='dryad'][@qualifier=$fdl]"/>
												<xsl:text>&#160;&#160;&#160;</xsl:text>
												<xsl:value-of select="$fdl-value"/>
												<xsl:choose>
												<xsl:when test="string($fdl-value) = '1'">
												<i18n:text>xmlui.DryadItemSummary.download</i18n:text>
												</xsl:when>
												<xsl:otherwise>
												<i18n:text>xmlui.DryadItemSummary.downloads</i18n:text>
							...
							<!-- This tries to load the metadata file - it works for handles, but the redirect for doi's doesn't seem to work (document
								function returns an empty node set but no error indication - hardcoded URL works in browser) -->
							<!--xsl:choose>
								<xsl:when test="contains($fileURL,'handle')">
									<xsl:variable name="mymeta" select="concat($myscheme,'://',$myHost,':',$myPort,'/metadata/handle/10255',substring-after($fileURL,'10255'),'/mets.xml' )"/>
								 	<xsl:for-each select="document($mymeta)">
										<xsl:variable name="fields" select="mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field"/>
								 		<xsl:copy-of select="$fields[@element='description']"/>
								 		<br/>
								 	</xsl:for-each>
								</xsl:when>
								
								<xsl:when test="contains($fileURL,'doi')">
									<xsl:variable name="doiURL"  select="concat($myscheme,'://',$myHost,':',$myPort,'/doi?redirect=doi:10.5061/dryad.962/1')"/>
									<xsl:variable name="doiTest" select="document(doiURL)"/-->  <!-- this is the document call that returns an empty set -->
									<!--xsl:if test="count($doiTest)=0">
										<xsl:value-of select="$doiURL"/>
										<xsl:text> Shouldn't be empty</xsl:text>
									</xsl:if>
									<br/>
								</xsl:when>
							</xsl:choose> 
								...
								<div style="margin-top: 2px; margin-bottom: 5px;">
									<a>
										<xsl:attribute name="href">
											...
											<xsl:text>?show=full</xsl:text>
										</xsl:attribute>
										<i18n:text>xmlui.DryadItemSummary.showFileFull</i18n:text>
									</a>
								</div>
								<br/>
							</xsl:if>
	...
	<xsl:template name="journal-lookup">
		...
		<xsl:choose>
			<xsl:when test='$journal-name = "The American Naturalist"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/amNatCover.png" alt="The American Naturalist cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Ecological Monographs"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/ecoMonoCover.png" alt="Ecological Monographs cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Evolution"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/evolutionCover.png" alt="Evolution cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Evolutionary Applications"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/evoApplCover.png" alt="Evolutionary Applications cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Heredity"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/heredityCover.png" alt="Heredity cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Journal of Evolutionary Biology"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/jEvoBioCover.png" alt="Journal of Evolutionary Biology cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Molecular Biology and Evolution"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/mbeCover.png" alt="Molecular Biology and Evolution cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Molecular Ecology"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/molecularEcoCover.png" alt="Molecular Ecology cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Molecular Phylogenetics and Evolution"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/molPhyloEvoCover.png" alt="Molecular Phylogenetics and Evolution cover"/>
				</a>
			</xsl:when>
			<xsl:when test='$journal-name = "Systematic Biology"'>
				<a href="http://sysbio.oxfordjournals.org/">
					<img id="journal-logo" src="/themes/Dryad/images/coverimages/systBioCover.png" alt="Systematic Biology cover"/>
				</a>
			</xsl:when>
		</xsl:choose>
	</xsl:template>

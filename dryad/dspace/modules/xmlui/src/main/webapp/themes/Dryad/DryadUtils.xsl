...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <!--  This template checks URLs passed to it to see whether they need to be 'localized' (also
    determined by a parameter that is passed to the template).  Localized in this context just
    means resolved locally.  For instance, a DOI that has been minted on the dev server will
    probably not be registered at dx.doi.org (so won't be able to be resolved there).  Instead,
    we want these locally registered DOIs to be resolved against our internal DOI database.
    This template rewrites those URLs to use a servlet that can resolve against the internal
    DOI database. It will do essentially the same thing for our deprecated Handles: instead
    of resolving against the handle.net, it will rewrite the URL to resolve to a DSpace URL. -->
	<xsl:template name="checkURL">
		<xsl:param name="url"/>
		<xsl:param name="localize"/>
		<xsl:choose>
            <xsl:when test="/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field[@mdschema='workflow'][@element='step'][@qualifier='reviewerKey'] or $meta[@element='identifier'][@qualifier='reviewerKey']">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field[@mdschema='workflow'][@element='step'][@qualifier='reviewerKey']"/>
                    </xsl:otherwise>
                </xsl:choose>
                <xsl:text>&amp;itemID=</xsl:text>
                <xsl:value-of select="substring-after(/mets:METS/@OBJEDIT, 'itemID=')"/>
                <xsl:text>&amp;doi=</xsl:text>
                <xsl:choose>
                    <xsl:when test="starts-with($url, 'http://dx.doi.org/10.5061/')">
						<xsl:value-of select="substring($url, 19)"/>
					</xsl:when>
				    <xsl:when test="starts-with($url, 'doi:10.5061/')">
				        <xsl:value-of select="."/>
				    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:when test="not(/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field[@mdschema='dc'][@element='date' ][@qualifier='accessioned'])">
                <!--This url can be used to redirect a user by using the doi identifier, only used in the workflow-->
                <xsl:value-of select="confman:getProperty('doi.service.url')"/>
            ...
            <xsl:when test="$localize">
                <xsl:choose>
				    ...
				    <xsl:when test="starts-with($url, 'doi:10.5061/')">
				        <xsl:value-of select="concat('/doi?redirect=', .)"/>
				    </xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$url"/>
					</xsl:otherwise>
			...
			<xsl:otherwise>
			    <xsl:choose>
			        <xsl:when test="starts-with($url, 'doi:10.5061/')">
			            <xsl:value-of select="concat('http://dx.doi.org/', substring($url, 5))"/>
			        </xsl:when>
			        <xsl:otherwise>
			            <xsl:value-of select="$url"/>
			        </xsl:otherwise>

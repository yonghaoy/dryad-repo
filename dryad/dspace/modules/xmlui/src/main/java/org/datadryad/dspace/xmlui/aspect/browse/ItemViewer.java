...
public class ItemViewer extends AbstractDSpaceTransformer implements
	...
	public void addBody(Body body) throws SAXException, WingException,
		...
		/*
		 * reference any parent items to create listing...

        if (item.getMetadata("dc.relation.ispartof").length > 0){
             ReferenceSet isPartOf;

            if (showFullItem(objectModel)) {
                isPartOf = itemRef.addReferenceSet(ReferenceSet.TYPE_DETAIL_VIEW, null, "isPartOf");
            } else {
                isPartOf = itemRef.addReferenceSet(ReferenceSet.TYPE_SUMMARY_VIEW, null, "isPartOf");
            }

            isPartOf.setHead(T_head_is_part_of);

            for (DCValue value : item.getMetadata("dc.relation.ispartof")) {
                if (dis != null) {
                    try
                    {
                        DSpaceObject obj = dis.resolve(context, value.value);
                        if (obj != null) {
                            isPartOf.addReference(obj);
                        }
                    }
                    catch(Exception e)
                    {
